 ...
 <h2 className="font-extrabold text-3xl my-3">Criar Task:</h2>
  <form onSubmit={handlesubmit} >
    ...
    <select value={category} onChange={(e) => setCategory(e.target.value)}
      className={`text-purple-50 rounded  outline-none p-2 border border-sky-700  bg-slate-800 focus:outline-none focus:border-sky-500 w-[32rem] flex col focus:ring-1 focus:ring-sky-500 py-2 mb-2 ${lineColor}`}>
      <option value="">Selecione uma categoria</option>
      <option value="Leve">Leve</option>
      <option value="Importante">Importante</option>
      <option value="Urgente">Urgente</option> 
    </select>
    <button className={`p-3 flex justify-  px-16 font-bold rounded bg-sky-800  hover:bg-sky-400 duration-500 ` }>CriarTask</button>
    </form>
